...
没有配置密钥时，在侧栏选择“装载演示研究”即可浏览离线演示。要运行实时研究，在首页填写行业、地域、时间范围和用途，创建在线研究任务；批准七维框架后，在“资料来源”页点击“开始自动网络检索”。复制 `.env.example` 为本地环境配置，或将同名字段写入 Streamlit Secrets。密钥只在服务端读取，不要提交到 Git。
